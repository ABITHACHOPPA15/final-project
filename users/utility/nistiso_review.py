...
def generate_encryption_key():
    ...
    with open("encryption_key1.key", "wb") as key_file:
        key_file.write(key)
...
def main_security_pipeline(f1, f2):
    setup_logging()
    path = os.path.join(settings.MEDIA_ROOT, 'review', f1)
    file1 = open(path, "r+")
    ...
    path2 = os.path.join(settings.MEDIA_ROOT, 'review', f2)
    file2 = open(path2, "r+")
    # Encryption & Decryption Demonstration (Confidentiality Control)
    sensitive_data2 = file2.read()  # "This is some highly sensitive information."
    print("\n--- Encrypting Sensitive Data ---")
    encrypted2 = encrypt_data(sensitive_data2)
    # with open(path2, "w") as f:
    #     f.write(encrypted2.decode())
    print(f"Encrypted Data: {encrypted}")
    ...
    print(f"Decrypted Data: {decrypted2}")

    # File Integrity Check
    print("\n--- Checking File Integrity ---")
    original_hash = check_file_integrity(path)
    print(f"Original file hash: {original_hash}")
    tampered_hash = check_file_integrity(path2)
    print(f"Tampered file hash: {tampered_hash}")

    if original_hash != tampered_hash:
        log_event(f"File {path} integrity check failed! Possible tampering detected.")
        result = "Integrity check failed! Possible tampering detected"
        print(result)
    else:
        log_event(f"File {path} passed integrity check.")
        result = "File integrity is intact."
        print(result)
    return result


# Execute the security pipeline
def star_process(f1, f2):
    # Generate encryption key if not already generated
    key = None
    if not os.path.exists("encryption_key1.key"):
        key = generate_encryption_key()
        print("Key is:", key)

    return main_security_pipeline(f1, f2)
